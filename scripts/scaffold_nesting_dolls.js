...
3. **Log Activity**:
   - Execute \`krusch_context_write_state\` with \`category: 'activity'\`, \`author_id: 'agent'\` and content: \`[${name}] <description>\`.
...
1. **Context Load (Project-Scoped)**:
   ...
   - Query \`krusch_context_search_memory(category: 'activity', query: '${name}')\`.
   - Query \`krusch_context_search_memory(category: 'lessons', query: '${name}')\`.
   - Query \`krusch_context_nugget_nudges(kinds: ['project', 'user'], query: '${name}')\`.
   - **Zero-Trust**: Execute \`krusch_context_search_code(project: '${name}')\` to verify codebase state.
